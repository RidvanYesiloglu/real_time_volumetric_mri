...
def mean_squared_err_2d(reconstructed, original, ax_cr_sg): # input, target
    if ax_cr_sg == 0:
        ...
        return ((reconstructed-original)**2).mean((1,3))
    elif ax_cr_sg == 2:
        return ((reconstructed-original)**2).mean((2,3))
def calc_psnrs_2d(reconstructed, original, ax_cr_sg):
    if ax_cr_sg == 0:
        ...
        return 20*np.log((original**2).max((1,3)))-10*np.log(((original-reconstructed)**2).mean((1,3)))
    elif ax_cr_sg == 2:
        return 20*np.log((original**2).max((2,3)))-10*np.log(((original-reconstructed)**2).mean((2,3)))
    
def main(output_im, ref_im, step, ax_cr_sg, pt_id, res_dir, plot_max_mse=False):
    ...
    ind_ims_dir = f'{res_dir}{gif_name}ind_ims'
    ...
    filenames = []
    for t in np.arange(0, ref_im.shape[0]):
