...
def gif_freq_actions(inps_dict, preruni_dict):
    ...
    for ax_cr_sg in [0,1,2]:
        plot_max_mse=True
        p = multiprocessing.Process(target = make_gif_of_rec_vs_gt.main, args=(output_im.cpu().detach().numpy().squeeze(), preruni_dict['main_module'].image.cpu().detach().numpy().squeeze(), 350, ax_cr_sg, args.pt, inps_dict['res_dir'], plot_max_mse,))
        p.start()
