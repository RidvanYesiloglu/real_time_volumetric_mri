...
def make_gif_frames(args, recs, refs, psnrs, ssims, jcs, ts, ax_cr_sg, sl_no, gif_dir, gif_name, t_st):
    ...
    for t in np.arange(0, recs.shape[1]):
        ...
        for i in range(nrows):
            for j in range(ncols):
                ...
                if j == 0:#-22
                    ...
                    if i == ((nrows-1)//2):
                        ax[i,j].text(-67,140, 'Coefficient of JC Reg. on Grid', color='black', size=17, ha="center", va="center", rotation='vertical', transform=ax[i,j].transData)
        ...
        plt.subplots_adjust(left=0.04, right=0.55, bottom=0.05, top=0.92, wspace=0.03, hspace=0.26)
        ...
        if args.conf == 'trn_w_trns':
            plt.suptitle(f"{im_type_str.capitalize()} Images vs JC-on-Grid and Temporal Continuity Loss Coefficients ({args.pt} - With Transformation NeRP - Sp Loss Coef.: {args.lambda_sp} - Time Point: {(t+t_st):3d})", size=19)
        else:
